...
async function syncExactNames() {
  ...
  for (const catId in CAT_VARIANTS) {
    ...
    // We only update names for items that have an Amazon image URL, to ensure perfect matching
    // Blinkit and local AI ones are already exact.
    if (cat.img && cat.brands) {
      console.log(`\n=== Checking Category ${catId} ===`);
      for (let i = 0; i < cat.brands.length; i++) {
        const brand = cat.brands[i];
        const currentImg = cat.img[i];
        
        if (currentImg && currentImg.includes('amazon.com')) {
          console.log(`Syncing name for Amazon product: ${brand}`);
          ...
          try {
            ...
            const exactData = await page.evaluate(() => {
              ...
              return img ? { name: img.alt, url: img.src } : null;
            });

            if (exactData && exactData.name) {
              // Clean up super long names
              let cleanName = exactData.name.split(',')[0].split('-')[0].trim();
              if (cleanName.length > 40) cleanName = cleanName.substring(0, 40) + '...';
              
              if (cleanName && cleanName !== brand) {
                console.log(`  Updated: ${brand} -> ${cleanName}`);
  ...
  await browser.close();
  console.log(`\nTotal names synced: ${totalUpdated}`);
  ...
  console.log('Updated ui/index.html with exact Amazon product names.');
}
