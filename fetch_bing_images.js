...
async function searchBingImage(query) {
  ...
  return new Promise((resolve, reject) => {
    https.get(url, options, res => {
      ...
      res.on('end', () => {
        // Look for murl":"..." which is Bing's direct image URL or turl":"..." which is thumbnail
        const match = data.match(/"turl":"([^"]+)"/);
    ...
    if (cat.img && cat.brands) {
      for (let i = 0; i < cat.brands.length; i++) {
        ...
        if (!url || url.startsWith('U')) {
          ...
          console.log(`Searching Bing Images for: ${brand}`);
          try {
            const newUrl = await searchBingImage(brand + ' product');
            if (newUrl) {
          ...
          await new Promise(r => setTimeout(r, 200)); // Be nice to Bing
        }
      }
    }
  }
